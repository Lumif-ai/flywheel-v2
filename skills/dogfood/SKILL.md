...
Verify `agent-browser` is available and the Playwright MCP server is responsive:

```bash
which agent-browser && agent-browser --version 2>&1 | head -1 || echo "MISSING"
```

If `agent-browser` is not found or MCP is not connected, stop and tell the user: "agent-browser CLI is required but not found. Ensure Playwright MCP is running."

---

## Step 0b: Input Validation

- **Target URL required:** If no URL is provided, ask for one. Do not default to a URL.
- **URL accessibility:** After opening the target URL, verify the page loads (check for non-error HTTP response via snapshot). If the page shows a connection error or blank screen, report it immediately rather than proceeding with broken-state testing.
...
| Failure | Response |
|---------|----------|
| App unreachable (connection refused, DNS failure) | Report the exact error from `agent-browser open`, ask user to verify the URL and that the app is running |
| Page load timeout (networkidle never reached) | Take a screenshot of whatever loaded, note the timeout in the report, and proceed with available content |
| Screenshot/recording failure (disk full, permissions) | Warn user that evidence capture failed, continue documenting issues in text, note missing screenshots in report |
| Session crash mid-test | Save the report as-is (incremental writes protect completed work), report how many issues were captured before the crash |
...
**Memory file:** `~/.claude/projects/-Users-sharan/memory/dogfood.md`
