...
**Default to the Lumif.ai design system** for company/personal presentations.
Read `~/.claude/design-guidelines.md` for full tokens (colors, typography, spacing).
Core brand: `#E94D35` coral accent, Inter font, clean/warm/professional aesthetic.
Apply brand tokens for internal, pitch, or company-facing decks. Ask/infer for other contexts.
...
YOUR FILES
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
  Presentation:     /absolute/path/to/[filename].html
                    [N] slides, [Style Name] style
...
- Animations: `.reveal` class timings
...
This skill learns user preferences over time to reduce repeat questions and deliver better results.

**Memory file:** Check the auto-memory directory for `frontend-slides.md`:
```bash
cat "$(find ~/.claude/projects -name 'frontend-slides.md' -path '*/memory/*' 2>/dev/null | head -1)" 2>/dev/null || echo "NOT_FOUND"
```

### Loading preferences (at skill start)

Before the design exploration phase, check for saved preferences. If found, load and auto-apply:

```
Learned preferences loaded:
├─ Preferred aesthetic: [e.g. "minimal dark", "bold maximalist"]
├─ Aspect ratio: [e.g. "16:9", "4:3"]
├─ Font preferences: [e.g. "loves Satoshi + JetBrains Mono"]
└─ Animation style: [e.g. "subtle transitions", "dramatic reveals"]
```

Skip any questions that have saved answers. Show what was auto-applied.

### What to save after each run

At the end of every presentation generation, update the memory file with any new or changed preferences:

- **Aesthetic direction** — which visual style the user chose or praised
- **Font pairings** — specific fonts the user approved or requested
- **Color palettes** — any custom colors or palette preferences
- **Animation preferences** — subtle vs dramatic, specific transition types preferred
- **Aspect ratio** — 16:9, 4:3, or custom
- **Content density** — how much text per slide the user prefers
- **Slide structure patterns** — preferred layouts (e.g. "always starts with a full-bleed image slide")
- **Feedback corrections** — anything the user asked to change (avoid repeating mistakes)

Use the Edit tool to update existing entries — never duplicate. Save to the auto-memory directory at `~/.claude/projects/-Users-sharan-Projects/memory/frontend-slides.md`.

### What NOT to save

- Content of specific presentations (session-specific)
- Temporary overrides the user explicitly marked as one-time
...
- If converting PPTX input: verify `python-pptx` is available (`python3 -c "import pptx" 2>/dev/null`). Abort with install instructions if missing.
- For HTML generation from scratch: no external dependencies required.

## Input Validation
- Confirm the user has provided either a topic/content outline or a PPTX file path. Reject empty requests.
- If PPTX conversion: verify the file exists and is a valid `.pptx` before attempting extraction.

## Error Handling
- **PPTX parse failure:** If `python-pptx` cannot read the file (corrupt or password-protected), surface the error and ask for a re-export or alternative file.
- **Asset embedding failure:** If images extracted from PPTX are missing or corrupted, skip the broken asset with a warning and continue generating remaining slides.
- **Viewport overflow:** If generated content exceeds viewport bounds, flag the offending slide and suggest content reduction.
