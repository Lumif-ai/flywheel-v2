...
Verify the gstack daemon is running or can auto-start:

```bash
$B status 2>&1 | head -1 || echo "DAEMON_DOWN"
```

If the daemon is not running and auto-start fails, check that the compiled binary exists (see SETUP below). If the binary is missing, trigger the setup flow.

---

## Step 0b: Input Validation

- **URL format:** Verify the URL includes a scheme (`http://` or `https://` or `file://`). Prepend `https://` if missing.
- **Local URLs:** For `localhost` or `127.0.0.1` URLs, verify the port is specified.
...
| Failure | Response |
|---------|----------|
| Daemon not running and binary missing | Tell user "gstack browse needs a one-time build" and trigger SETUP flow |
| Page load failure (DNS error, connection refused) | Report the exact error from `$B goto`, suggest checking the URL or that the server is running |
| Content extraction returns empty | Try `$B screenshot` to visually verify the page loaded, report if the page is blank or requires JS rendering |
| Port conflict (9400-9410 all occupied) | Run `$B stop` to clean up stale daemons, then retry |
