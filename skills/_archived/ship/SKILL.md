...
**Memory file:** `~/.claude/projects/-Users-sharan/memory/ship.md`
...
- Verify `git` is available and current directory is a git repository. Abort if not.
- Verify `gh` CLI is installed (`which gh`) and authenticated (`gh auth status`). Abort with install/auth instructions if missing.
- Verify a remote named `origin` is configured (`git remote get-url origin`). Abort if no remote.

## Input Validation
- Confirm the current branch is not `main`. Abort with "Ship from a feature branch."
- Verify the `VERSION` file exists in the repo root. Abort if missing -- version bump cannot proceed.
- Check for merge conflicts in progress (`git ls-files -u | head -1`). Abort if unmerged files exist.

## Error Handling
- **Merge conflicts:** If `git merge origin/main` produces conflicts that cannot be auto-resolved, stop and display the conflicting files. Do not proceed.
- **CI/test failures:** If `bin/test-lane` or `npm run test` fails, stop and show the failure output. Do not proceed to version bump.
- **Push rejected:** If `git push` is rejected (force-push required, branch protection), surface the error and suggest rebasing or checking branch protection rules.
