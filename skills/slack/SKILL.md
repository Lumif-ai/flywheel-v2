...
Verify the Playwright MCP server is connected before any browser automation:

```bash
agent-browser snapshot -i 2>&1 | head -1
```

If the command fails or returns a connection error, stop and tell the user: "Playwright MCP is not connected. Please ensure the MCP server is running."

---

## Step 0b: Input Validation

- **Workspace URL:** If user provides a workspace URL, verify it matches `*.slack.com` pattern. Reject non-Slack URLs.
- **Channel name:** If user asks to navigate to a specific channel, confirm the channel name is provided before attempting navigation. Do not guess channel names.
- **Port for existing session:** If connecting to an existing browser, confirm port number (default 9222) is specified or use default.
...
| Failure | Response |
|---------|----------|
| Auth failure (Slack login wall, session expired) | Take a screenshot of the login screen, tell user their session has expired, ask them to re-authenticate in the browser |
| Channel not found (click leads to wrong place or element missing) | Re-snapshot the sidebar, search for the channel name, report if it does not exist in the visible list |
| Message send failure (compose box not found, send button unresponsive) | Screenshot the current state, report what went wrong, do not retry blindly |
| Connection refused on port 9222 | Tell user no existing Slack browser session was found, offer to open a new one with `agent-browser open` |
...
**Memory file:** `~/.claude/projects/-Users-sharan/memory/slack.md`
