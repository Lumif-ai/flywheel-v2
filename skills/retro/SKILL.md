...
**Memory file:** `~/.claude/projects/-Users-sharan/memory/retro.md`
...
- Verify current directory is a git repository (`git rev-parse --is-inside-work-tree`). Abort with "Run /retro from within a git repository." if not.
- Verify `origin/main` exists (`git rev-parse --verify origin/main`). Abort if remote is not configured.

## Input Validation
- After parsing the time window argument, verify commits exist in that range (`git log origin/main --since="<window>" --oneline | head -1`). If zero commits, report "No commits found in the last <window>. Try a wider window (e.g., /retro 14d)." and stop.
- Verify the git repo has at least one commit on `origin/main` before running any analysis.

## Error Handling
- **No commits in range:** Report the empty window clearly and suggest `/retro 14d` or `/retro 30d` as alternatives. Do not generate an empty retro.
- **Git fetch failure:** If `git fetch origin main` fails (network issue, no remote), warn and proceed with local `main` data, noting results may be stale.
- **Retro history write failure:** If `.context/retros/` cannot be created or written to, output the retro to the conversation and warn that the JSON snapshot was not saved.
