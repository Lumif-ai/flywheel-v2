...
- Verify the user has provided a plan file, document, or inline plan text to review. Abort with "Provide a plan file path or paste the plan to review." if nothing is given.
- If a file path is provided: verify the file exists and is readable before starting the review.

## Input Validation
- Confirm the plan file is non-empty and contains technical content (architecture, code references, or implementation steps). If it appears to be a business-only document with no technical substance, suggest `/plan-ceo-review` instead.
- If the plan references a codebase: verify the current directory is a git repo before running retrospective checks or code references.

## Error Handling
- **Plan file not found:** Surface the exact path that failed and ask for correction. Do not proceed with a partial review.
- **Missing architecture sections:** If the plan lacks architecture, data flow, or implementation detail, flag it upfront in Step 0 rather than producing a shallow review.
- **Git context failure:** If git commands fail (not a repo, no history), skip the retrospective learning step gracefully and note that prior review history is unavailable.
