...
- If a file path is provided: verify the file exists and is readable before starting the system audit.

## Input Validation
- Confirm the plan file is readable and non-empty. If the file is empty or cannot be parsed, stop and ask the user for the correct path.
- If the plan references a git repository (for the system audit), verify the current directory is a git repo before running git commands.

## Error Handling
- **Plan file not found:** If the provided path does not exist, surface the exact path that failed and ask the user to correct it. Do not proceed with a partial review.
- **File too large to process:** If the plan exceeds context limits, warn the user and offer to review it in sections (e.g., by section headings).
- **Git audit failure:** If git commands fail during the pre-review system audit (not a repo, no remote), skip the audit gracefully and note that system context is unavailable.
